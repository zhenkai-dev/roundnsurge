...
@section('listing')
    ...
    @component('member.shared.listing.listing-header-component', ['module' => \App\Member::class])
    ...
    @component('member.shared.listing.filter-form')
        @slot('action') {{ route('member.member.index') }} @endslot
        @slot('inputs')
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach ($members as $member)
                        ...
                        <tr>
                            <td><a href="{{ route('member.member.edit', $member->getId()) }}">{{ $member->getName() }}</a></td>
                            ...
                            <td class="text-center">
                                @if (!Auth::user()->can(\App\Enumeration\PolicyActionEnum::UPDATE, $member))
                                    {!! edit_icon_muted() !!}
                                @else
                                    {!! edit_icon_link(route('member.member.edit', $member->getId())) !!}
                                @endif

                                @if (!Auth::user()->can(\App\Enumeration\PolicyActionEnum::DELETE, $member))
                                    {!! delete_icon_muted() !!}
                                @else
                                    {!! delete_icon_link(route('member.member.destroy', $member->getId())) !!}
                                @endif
                            </td>
                        </tr>
                    @endforeach
