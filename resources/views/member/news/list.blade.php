...
@section('listing')
    ...
    @component('member.shared.listing.listing-header-component', ['module' => \App\News::class])
    ...
    @component('member.shared.listing.filter-form')
        @slot('action') {{ route('member.news.index') }} @endslot
        @slot('inputs')
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach ($newsList as $news)
                        ...
                        <tr>
                            <td>
                                <a href="{{ route('member.news.edit', $news->getId()) }}">{{ $news->newsTranslation->getName() }}</a>
                            ...
                            <td class="text-center">
                                @if (!Auth::user()->can(\App\Enumeration\PolicyActionEnum::UPDATE, $news))
                                    {!! edit_icon_muted() !!}
                                @else
                                    {!! edit_icon_link(route('member.news.edit', $news->getId())) !!}
                                @endif

                                @if (!Auth::user()->can(\App\Enumeration\PolicyActionEnum::DELETE, $news))
                                    {!! delete_icon_muted() !!}
                                @else
                                    {!! delete_icon_link(route('member.news.destroy', $news->getId())) !!}
                                @endif
                            </td>
                        </tr>
                    @endforeach
