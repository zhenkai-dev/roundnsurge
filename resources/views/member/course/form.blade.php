...
@section('form')
    ...
    @component('member.shared.form.form-header-component', ['module' => \App\Course::class])
    ...
    <form class="form-horizontal form-validation" action="{{ is_create() ? route('member.course.store') : route('member.course.update', $course->id) }}" method="post" enctype="multipart/form-data">
        @if (is_edit())
            {{ method_field('PATCH') }}
        @endif
