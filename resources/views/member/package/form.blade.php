...
@section('form')
    ...
    <form class="form-horizontal form-validation" action="{{ is_create() ? route('member.package.store') : route('member.package.update', $package->id) }}" method="post" enctype="multipart/form-data">
        @if (is_edit())
            {{ method_field('PATCH') }}
        @endif
        ...
        @component('member.shared.form.form-group')
            ...
            @slot('input')
                @component('member.shared.input.text-group-component')
        ...
        @include('member.shared.form.template.save-after-action-component', ['noCreate' => true])
