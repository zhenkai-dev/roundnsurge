...
@section('listing')
    ...
    @component('member.shared.listing.listing-header-component', ['module' => \App\Package::class])
        @slot('total') {{ $packages->total() }} @endslot
    @endcomponent

    @component('member.shared.listing.filter-form')
        @slot('action') {{ route('member.package.index') }} @endslot
        @slot('inputs')
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach ($packages as $package)
                        ...
                        <tr>
                            <td>
                                <a href="{{ route('member.package.edit', $package->getId()) }}">{{ $package->packageTranslation->getName() }}</a>
                            ...
                            <td class="text-center">
                                @if (!Auth::user()->can(\App\Enumeration\PolicyActionEnum::UPDATE, $package))
                                    {!! edit_icon_muted() !!}
                                @else
                                    {!! edit_icon_link(route('member.package.edit', $package->getId())) !!}
                                @endif
                            </td>
                        </tr>
                    @endforeach
