...
@section('listing')
    ...
    @component('admin.shared.listing.listing-header-component', ['module' => \App\User::class])
        @slot('total') {{ $users->total() }} @endslot
        @slot('addNewUrl') {{ route('admin.user.create') }}  @endslot
    @endcomponent

    @component('admin.shared.listing.filter-form')
        @slot('action') {{ route('admin.user.index') }} @endslot
        @slot('inputs')
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach ($users as $user)
                        ...
                        <tr>
                            <td><a href="{{ route('admin.user.edit', $user->getId()) }}">{{ $user->getName() }}</a></td>
                            <td>{{ $user->getUsername() }}
                            </td>
                            <td>{{ $user->getEmail() }}</td>
                            ...
                            <td class="text-center">
                                @if (!Auth::user()->can(\App\Enumeration\PolicyActionEnum::UPDATE, $user))
                                    {!! edit_icon_muted() !!}
                                @else
                                    {!! edit_icon_link(route('admin.user.edit', $user->getId())) !!}
                                @endif

                                @if ($user->isPersist() || !Auth::user()->can(\App\Enumeration\PolicyActionEnum::DELETE, $user))
                                    {!! delete_icon_muted() !!}
                                @else
                                    {!! delete_icon_link(route('admin.user.destroy', $user->getId())) !!}
                                @endif
                            </td>
                        </tr>
                    @endforeach
