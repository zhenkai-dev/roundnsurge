...
@section('form')
    ...
    @component('admin.shared.form.form-header-component', ['module' => \App\User::class])
        @slot('addNewUrl') {{ route('admin.user.create') }}  @endslot
    @endcomponent

    @php /* @var \App\User $user */ @endphp
    <form class="form-horizontal form-validation" action="{{ is_create() ? route('admin.user.store') : route('admin.user.update', $user->id) }}" method="post" enctype="multipart/form-data">
        @if (is_edit())
            {{ method_field('PATCH') }}
        @endif
