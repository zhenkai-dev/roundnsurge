...
@section('listing')
    ...
    @component('admin.shared.listing.listing-header-component', ['module' => \App\Page::class])
        @slot('total') {{ $pages->total() }} @endslot
        @slot('addNewUrl') {{ route('admin.page.create') }}  @endslot
    @endcomponent

    @component('admin.shared.listing.filter-form')
        @slot('action') {{ route('admin.page.index') }} @endslot
        @slot('inputs')
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach ($pages as $page)
                        ...
                        <tr>
                            <td>
                                <a href="{{ route('admin.page.edit', $page->getId()) }}">{{ $page->pageTranslation->getName() }}</a>
                            ...
                            <td class="text-center">
                                @if (!Auth::user()->can(\App\Enumeration\PolicyActionEnum::UPDATE, $page))
                                    {!! edit_icon_muted() !!}
                                @else
                                    {!! edit_icon_link(route('admin.page.edit', $page->getId())) !!}
                                @endif

                                @if ($page->isPersist() || !Auth::user()->can(\App\Enumeration\PolicyActionEnum::DELETE, $page))
                                    {!! delete_icon_muted() !!}
                                @else
                                    {!! delete_icon_link(route('admin.page.destroy', $page->getId())) !!}
                                @endif
                            </td>
                        </tr>
                    @endforeach
