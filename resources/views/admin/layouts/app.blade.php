...
<header class="app-header navbar">
    ...
    <ul class="nav navbar-nav d-md-down-none">
        ...
        <li class="nav-item px-3">
            <a class="nav-link" href="{{ route('admin.home') }}">{{ __('common.dashboard') }}</a>
        </li>

        @if (can_access_module(\App\User::class))
            <li class="nav-item px-3">
                <a class="nav-link" href="{{ route('admin.user.index') }}">{{ trans_choice('entity.user', 2) }}</a>
            </li>
        @endif

        @if (can_access_module(\App\Setting::class))
            <li class="nav-item px-3">
                <a class="nav-link" href="{{ route('admin.setting.edit') }}">{{ trans_choice('entity.setting', 1) }}</a>
            </li>
        @endif
    </ul>
    <ul class="nav navbar-nav ml-auto">
        <li class="nav-item dropdown pr-3">
            ...
            <div class="dropdown-menu dropdown-menu-right">
                ...
                <a class="dropdown-item" href="{{ route('admin.account.index') }}"><i class="fa fa-user"></i> {{ __('account.profile') }}</a>
                <a class="dropdown-item" href="{{ route('admin.password.index') }}"><i class="fa fa-key"></i> {{ __('account.change_password') }}</a>
        ...
        <li class="nav-item pr-3">
            <a class="nav-link" href="{{ url('/') }}" target="_blank">
                <i class="icon-globe"></i> {{ __('common.view_front') }}
            </a>
        </li>
    </ul>
</header>
            ...
            <nav class="sidebar-nav">
                <ul class="nav">
                    ...
                    @component('admin.shared.sidebar.item-component')
                        @slot('url') {{ route('admin.home') }} @endslot
                    ...
                    @if (can_access_module(\App\Menu::class))
                        @component('admin.shared.sidebar.item-component')
                            @slot('url') {{ route('admin.menu.index') }} @endslot
                    ...
                    @if (can_access_module(\App\Banner::class))
                        @component('admin.shared.sidebar.item-component')
                            @slot('url') {{ route('admin.banner.index') }} @endslot
                    ...
                    @if (can_access_module(\App\User::class))
                        @component('admin.shared.sidebar.item-component')
                            @slot('url') {{ route('admin.user.index') }} @endslot
                    ...
                    @if (can_access_module(\App\Setting::class))
                        @component('admin.shared.sidebar.item-component')
                            @slot('url') {{ route('admin.setting.edit') }} @endslot
    ...
    @include('admin.shared.delete-dialog-component')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('[data-name="pageSize"]').change(function () {
                ...
                var value = $this.val();
                $.ajax({
                    type: 'POST', url: '{{ route('admin.setting.updatePageSize') }}', dataType: 'json', data : { 'pageSize': value },
                    complete: function () {
            ...
            $.ajax({
                type: 'POST', url: '{{ route('admin.setting.updateClientTimezone') }}', dataType: 'json', data : { 'timezone': moment.tz.guess() }
            });
