...
@section('listing')
    ...
    @component('admin.shared.listing.listing-header-component', ['module' => \App\Banner::class])
                        ...
                        <div class="dd-handle clearfix">
                            ...
                            <div class="pull-right dd-nodrag">
                                @if (!Auth::user()->can(\App\Enumeration\PolicyActionEnum::UPDATE, $banner))
                                    {!! edit_icon_muted() !!}
                                @else
                                    {!! edit_icon_link(route('admin.banner.edit', $banner->getId())) !!}
                                @endif

                                @if (!Auth::user()->can(\App\Enumeration\PolicyActionEnum::DELETE, $banner))
                                    {!! delete_icon_muted() !!}
                                @else
                                    {!! delete_icon_link(route('admin.banner.destroy', $banner->getId())) !!}
                                @endif
                            </div>
                        </div>
                    </li>
    ...
    <script>
        $(document).ready(function() {
            var updateOutput = function (e) {
                ...
                $.ajax({
                    type: 'PUT', url: '{{ route('admin.banner.sortable') }}', dataType: 'json', data: param,
